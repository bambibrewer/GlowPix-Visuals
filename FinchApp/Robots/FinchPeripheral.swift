		...
		DispatchQueue.main.async {
			...
			self.syncTimer =
				Timer.scheduledTimer(timeInterval: self.syncInterval, target: self,
                                     selector: #selector(self.syncronizeOutputs),  //NOTE!! Was HummingbirdPeripheral.syncronizeOutputs but that caused a memory problem!?!
	...
	*/
	...
	func setTriLED(port: UInt, intensities: BBTTriLED) -> Bool {
		...
		self.conditionHelper(condition: self.writtenCondition, holdLock: false,
            ...
            self.nextOutputState.trileds[i] = BBTTriLED(r, g, b)
		})
		
		self.writtenCondition.unlock()
		
		print("exit setTriLED")
		return true
	}
    ...
    @objc func syncronizeOutputs() {
		...
		if self.initialized && (self.lastWriteWritten || timeout)  && shouldSync {
			...
			*/
            ...
            if changeOccurred {
                print("Sending change: \(command.map({return $0}))")
            }
		}
