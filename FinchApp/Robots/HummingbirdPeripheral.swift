...
//		Thread.sleep(forTimeInterval: 4) //make sure that the HB is booted up
		...
		guard versionArray[2] >= 2 &&
			...
			BLE_Manager.delegate?.robotFirmwareIncompatible(id: self.id, firmware: self.firmwareVersionString)
		...
		if versionArray[3] == 1 && versionArray[4] >= 105 {
			BLE_Manager.delegate?.robotFirmwareStatus(id: self.id, status: "old")
