...
class BLECentralManager: NSObject, CBCentralManagerDelegate {
    ...
    private var displayDiscoveredPeripherals: [(robot:CBPeripheral,ss:NSNumber)]
    ...
    public var displayDevices: [(robot: CBPeripheral, ss: NSNumber)] {
        return self.displayDiscoveredPeripherals
    }
	...
	func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
		...
		case .searchingScan:
			...
			if self.discoveredPeripherals.keys.contains(id) {
                ...
                self.displayDiscoveredPeripherals.append((peripheral, RSSI))
			}
	...
	func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
		...
		self.currentlyConnecting = robotInit(peripheral, {rbt in
			...
			if self.oughtToBeConnected.keys.contains(id) {
				...
				self.delegate?.robotUpdateStatus(id: id, connected: true)
				self.currentlyConnecting = 5
			} else {
//				self.disconnect(byID: id)
			}
	...
	*/
	func centralManager(_ central: CBCentralManager,
		...
		self.delegate?.robotUpdateStatus(id: id, connected: false)
	}
